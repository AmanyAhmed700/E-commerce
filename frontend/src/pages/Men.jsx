...
const Men = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // تم تغيير localhost إلى رابط Railway
        const res = await axios.get(`${API_URL}/api/products?category=men`);
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6 text-center">Men Section</h1>
      {products.length === 0 ? (
        <p className="text-gray-500 text-center">No products available in this category.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => {
            // معالجة رابط الصورة ليقرأ من السيرفر بدلاً من localhost
